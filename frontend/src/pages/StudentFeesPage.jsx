...
import { DollarSign, CheckCircle2, AlertTriangle, FileText, Printer } from 'lucide-react'
...
export default function StudentFeesPage() {
  ...
  useEffect(() => {
    if (user && user.student_id) {
      fetchStudentFeeSummary(user.student_id)
    }
  }, [user])
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-black text-schoolGreen mb-2">My Fees & Dues</h1>
        <p className="text-gray-600 font-medium">
          View billing details, scholarship applications, and receipt copies
        </p>
      </div>
            ...
            <div>
              ...
              {total_balance > 0 && (
                <p className="text-[10px] text-red-500 font-bold mt-1.5 uppercase flex items-center gap-1 bg-red-50 px-2 py-0.5 border border-red-100 rounded w-fit">
                  <AlertTriangle size={12} /> Outstanding Balance
                </p>
              )}
            </div>
